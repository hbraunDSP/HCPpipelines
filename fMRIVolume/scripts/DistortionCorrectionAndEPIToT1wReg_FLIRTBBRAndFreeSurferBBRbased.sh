...
NoFreeSurfer=`getopt1 "--nofreesurfer" $@`


#With new TOPUP options, we need to know what the original method 
# was if we are going to resume
OrigDistortionCorrection="$DistortionCorrection"
if [[ "$DistortionCorrection" = *@* ]] ; then
	OrigDistortionCorrection=${DistortionCorrection/*@/""}
...
case "$BiasCorrection" in
    ...
    SEBASED)
        if [[ "$OrigDistortionCorrection" != "${SPIN_ECHO_METHOD_OPT}" ]]
        then
...
case $DistortionCorrection in
    ...
    ${RESUME_METHOD_OPT})
        ...
        log_Msg "Resuming with original method: $OrigDistortionCorrection"
        DistortionCorrection="$OrigDistortionCorrection"
        ;;
    ...
    ${RESUME_T1REG_METHOD_OPT})
        ...
        log_Msg "Resuming after T1w with original method: $OrigDistortionCorrection"
        DistortionCorrection="$OrigDistortionCorrection"
	;;
...
if [ "X${NoFreeSurfer}" = "X" ]; then
  ...
  log_Msg "SKIPPING FREESURFER BBR. --nofreesurfer option was given"
  echo "1 0 0 0" > ${WD}/fMRI2str_refinement.mat
  echo "0 1 0 0" >> ${WD}/fMRI2str_refinement.mat
  echo "0 0 1 0" >> ${WD}/fMRI2str_refinement.mat
  echo "0 0 0 1" >> ${WD}/fMRI2str_refinement.mat
fi
