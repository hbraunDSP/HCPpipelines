...
#If we are using PDw instead of actual T2w, we need to transform the 
#fake T2w in T1w_acpc space back to its "raw" position (eg aligned to 
#the raw PDw.nii.gz)
PDwFolder="$StudyFolder"/"$Subject"/PDw
mkdir -p ${T2wFolder}/xfms
if [ -d ${PDwFolder} ] && [ ! -e ${T2wFolder}/xfms/acpc.mat ] ; then
	cp -f ${PDwFolder}/xfms/acpc.mat ${T2wFolder}/xfms/acpc.mat
fi
if [ -d ${PDwFolder} ] && [ ! -e ${T2wFolder}/${OrginalT2wImage} ]; then
	convert_xfm -omat ${PDwFolder}/xfms/PDw_orig_to_T1w_acpc.mat -concat ${PDwFolder}/xfms/T2wToT1wReg/T2w2T1w.mat ${PDwFolder}/xfms/acpc.mat
	convert_xfm -omat ${PDwFolder}/xfms/T1w_acpc_to_PDw_orig.mat -inverse ${PDwFolder}/xfms/PDw_orig_to_T1w_acpc.mat
	applywarp --interp=spline --in=${T1wFolder}/${T2wImage}--ref=${PDwFolder}/PDw --premat=${PDwFolder}/xfms/T1w_acpc_to_PDw_orig.mat --out=${T2wFolder}/${OrginalT2wImage}
fi
