function MSMregression(inputspatialmaps,inputdtseries,inputweights,outputspatialmaps,outputweights,wbcommand,Method,Params,VN,nTPsForSpectra,BC,inputvoldata)
...
vmask=[];
betaICAvol=[];
if(~exist('inputvoldata','var') && exist(inputvoldata,'file'))
    %BOvol=loadnii2cifti(inputvoldata);
    
    [BOvol,volsize,voxsize]=read_avw(inputvoldata);
    %volsize=size(BOvol);
    volmask=~all(BOvol==0,4);
    vmask=reshape(volmask,[],1);
    BOvol=reshape(single(BOvol),[],volsize(4));
    BOvol=BOvol(vmask,:);

    betaICAvol = ((pinv(NODEts) * demean(BOvol')))';
    OUTBOvol=single(zeros([size(vmask,1) size(NODEts,2)]));
    OUTBOvol(vmask,:)=betaICAvol;
    OUTBOvol=reshape(OUTBOvol,[volsize(1:3) size(NODEts,2)]);
...
if (length(Method) > 2) && strcmp(Method(3),'N')
    ...
    if(~isempty(betaICAvol))
        GMvolmask=true(size(betaICAvol,1),1);
        betaICAvolmean=mean(betaICAvol(GMvolmask,:));
        betaICAvolstd=std(betaICAvol(GMvolmask,:));
        
        betaICAvolnorm = ((((betaICAvol-repmat(betaICAvolmean,length(betaICAvol),1))./repmat(betaICAvolstd,length(betaICAvol),1)).*repmat(GMstd,length(betaICAvol),1))+repmat(GMmean,length(betaICAvol),1));
        OUTBOvol=single(zeros([size(vmask,1) size(NODEts,2)]));
        OUTBOvol(vmask,:)=betaICAvolnorm;
        OUTBOvol=reshape(OUTBOvol,[volsize(1:3) size(NODEts,2)]);
        outvolfile=[outputspatialmaps '.nii'];
        save_avw(OUTBOvol,outvolfile,'f',voxsize);
    end
end
