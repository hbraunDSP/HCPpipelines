...
main() 
{
	...
	local GrayordinatesSpaceDIRs="${g_grayordinates_space_dir}"
	log_Msg "GrayordinatesSpaceDIRs: ${GrayordinatesSpaceDIRs}"
	...
	GrayordinatesSpaceDIRs=`echo ${GrayordinatesSpaceDIRs} | sed 's/@/ /g'`
	log_Msg "After delimeter substitution, GrayordinatesSpaceDIRs: ${GrayordinatesSpaceDIRs}"
	...
	for Hemisphere in L R ; do
		...
		for LowResMesh in ${LowResMeshes} ; do
			...
			GrayordinatesSpaceDIR=`echo ${GrayordinatesSpaceDIRs} | cut -d " " -f ${i}`
		...
		for Mesh in ${LowResMeshes} ; do
			...
			for Surface in white midthickness pial ; do
			    ...
			    surface_in=${CommonAtlasFolder}/${GroupAverageName}.${Hemisphere}.${Surface}${RegSTRING}.${HighResMesh}k_fs_LR.surf.gii
			    sphere_in=${CommonAtlasFolder}/${GroupAverageName}.${Hemisphere}.sphere${SpecRegSTRING}.${HighResMesh}k_fs_LR.surf.gii
			    sphere_out=${CommonFolder}/${GroupAverageName}.${Hemisphere}.sphere.${Mesh}k_fs_LR.surf.gii

				${Caret7_Command} -surface-resample ${surface_in} ${sphere_in} ${sphere_out} BARYCENTRIC ${surface_out}
		...
		for Subject in ${Subjlist} ; do
			...
			if [ "$Mesh" = "${HighResMesh}" ] ; then
				...
				for Hemisphere in L R ; do
					...
					#rm ${AtlasFolder}/${Subject}.${Hemisphere}.midthickness${RegSTRING}_va.${Mesh}k_fs_LR.shape.gii
				done
			else      
				i=1
				for LowResMesh in ${LowResMeshes} ; do
				...
				for Hemisphere in L R ; do
					...
					${Caret7_Command} -surface-vertex-areas ${Folder}/${Subject}.${Hemisphere}.midthickness${RegSTRING}.${Mesh}k_fs_LR.surf.gii ${Folder}/${Subject}.${Hemisphere}.midthickness${RegSTRING}_va.${Mesh}k_fs_LR.shape.gii
				done
				...
				${Caret7_Command} -cifti-create-dense-scalar ${Folder}/${Subject}.midthickness${RegSTRING}_va.${Mesh}k_fs_LR.dscalar.nii -left-metric ${Folder}/${Subject}.L.midthickness${RegSTRING}_va.${Mesh}k_fs_LR.shape.gii -roi-left ${MNIFolder}/${Subject}.L.atlasroi.${Mesh}k_fs_LR.shape.gii -right-metric ${Folder}/${Subject}.R.midthickness${RegSTRING}_va.${Mesh}k_fs_LR.shape.gii -roi-right ${MNIFolder}/${Subject}.R.atlasroi.${Mesh}k_fs_LR.shape.gii
				for Hemisphere in L R ; do
					rm ${MNIFolder}/${Subject}.${Hemisphere}.midthickness${RegSTRING}_va.${Mesh}k_fs_LR.shape.gii
					rm ${Folder}/${Subject}.${Hemisphere}.midthickness${RegSTRING}_va.${Mesh}k_fs_LR.shape.gii
				done
			fi
