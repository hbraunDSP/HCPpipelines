...
if [ a = b ]; then
mri_convert -rl "$mridir"/T1w_hires.nii.gz "$mridir"/wm.mgz "$mridir"/wm.nii.gz
fslmaths "$mridir"/wm.nii.gz -thr 95 -uthr 115 "$mridir"/wm.nii.gz
wmMean=`fslstats "$mridir"/T1w_hires.nii.gz -k "$mridir"/wm.nii.gz -M`
...
fslmaths "$mridir"/T2w_hires.nii.gz -div `fslstats "$mridir"/T2w_hires.nii.gz -k "$mridir"/wm.nii.gz -M` -mul 57 "$mridir"/T2w_hires.norm.nii.gz -odt float
...
cp "$mridir"/aseg.presurf.mgz "$mridir"/aseg.mgz

#For mris_make_surface with correct arguments #Could go from 3 to 2 potentially...
mris_make_surfaces -nsigma_above 2 -nsigma_below 3 -aseg aseg -filled filled -wm wm -mgz -sdir $SubjectDIR -orig white -nowhite -orig_white white -orig_pial pial -T2dura "$mridir"/T2w_hires.norm -T1 T1w_hires.norm -output .T2 $SubjectID lh
mris_make_surfaces -nsigma_above 2 -nsigma_below 3 -aseg aseg -filled filled -wm wm -mgz -sdir $SubjectDIR -orig white -nowhite -orig_white white -orig_pial pial -T2dura "$mridir"/T2w_hires.norm -T1 T1w_hires.norm -output .T2 $SubjectID rh

fi
#mri_surf2surf --s $SubjectID --sval-xyz pial.T2 --reg $regII $mridir/wm.mgz --tval-xyz $mridir/wm.mgz --tval pial --hemi lh
#mri_surf2surf --s $SubjectID --sval-xyz pial.T2 --reg $regII $mridir/wm.mgz --tval-xyz $mridir/wm.mgz --tval pial --hemi rh

cp $SubjectDIR/$SubjectID/surf/lh.pial.T2 $SubjectDIR/$SubjectID/surf/lh.pial
cp $SubjectDIR/$SubjectID/surf/rh.pial.T2 $SubjectDIR/$SubjectID/surf/rh.pial
...
mris_make_surfaces -variablesigma ${VARIABLESIGMA} -white NOWRITE -aseg aseg -orig white -filled filled -wm wm -sdir $SubjectDIR -mgz -T1 T1w_hires.greynorm "$SubjectID" lh
mris_make_surfaces -variablesigma ${VARIABLESIGMA} -white NOWRITE -aseg aseg -orig white -filled filled -wm wm -sdir $SubjectDIR -mgz -T1 T1w_hires.greynorm "$SubjectID" rh
...
mris_make_surfaces -nsigma_above 2 -nsigma_below 3 -aseg aseg -filled filled -wm wm -mgz -sdir $SubjectDIR -orig white -nowhite -orig_white white -orig_pial pial -T2dura "$mridir"/T2w_hires.norm -T1 T1w_hires.norm -output .T2.two $SubjectID lh
mris_make_surfaces -nsigma_above 2 -nsigma_below 3 -aseg aseg -filled filled -wm wm -mgz -sdir $SubjectDIR -orig white -nowhite -orig_white white -orig_pial pial -T2dura "$mridir"/T2w_hires.norm -T1 T1w_hires.norm -output .T2.two $SubjectID rh
...
cp $SubjectDIR/$SubjectID/surf/lh.pial.T2.two $SubjectDIR/$SubjectID/surf/lh.pial
cp $SubjectDIR/$SubjectID/surf/rh.pial.T2.two $SubjectDIR/$SubjectID/surf/rh.pial
