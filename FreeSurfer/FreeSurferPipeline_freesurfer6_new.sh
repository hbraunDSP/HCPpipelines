...
fslmaths "$T1wImageFile" -div $Mean -mul 150 -abs "$T1wImageFile"_fs.nii.gz

#Initial Recon-all Steps
log_Msg "Initial Recon-all Steps"

expert_file=${HCPPIPEDIR_Config}/hires.opts
hires_options="-hires -expert ${expert_file}"
...
recon-all -i "$T1wImageFile"_fs.nii.gz -subjid $SubjectID -sd $SubjectDIR -motioncor -talairach -nuintensitycor -normalization ${hires_options} ${seed_cmd_appendix}

# Generate brain mask
mri_convert "$T1wImageBrainFile".nii.gz "$SubjectDIR"/"$SubjectID"/mri/brainmask.mgz --conform_min
mri_em_register -mask "$SubjectDIR"/"$SubjectID"/mri/brainmask.mgz "$SubjectDIR"/"$SubjectID"/mri/nu.mgz $FREESURFER_HOME/average/RB_all_2014-08-21.gca "$SubjectDIR"/"$SubjectID"/mri/transforms/talairach_with_skull.lta
mri_watershed -T1 -brain_atlas $FREESURFER_HOME/average/RB_all_withskull_2014-08-21.gca "$SubjectDIR"/"$SubjectID"/mri/transforms/talairach_with_skull.lta "$SubjectDIR"/"$SubjectID"/mri/T1.mgz "$SubjectDIR"/"$SubjectID"/mri/brainmask.auto.mgz 
...
recon-all -subjid $SubjectID -sd $SubjectDIR -autorecon2 -nosmooth2 -noinflate2 -nocurvstats -nosegstats -openmp ${num_cores} -hires ${seed_cmd_appendix}
...
"$PipelineScripts"/FreeSurferHiresWhite_freesurfer6_new.sh "$SubjectID" "$SubjectDIR" "$T1wImage" "$T2wImage"

#Intermediate Recon-all Steps
log_Msg "Intermediate Recon-all Steps"
recon-all -subjid $SubjectID -sd $SubjectDIR -smooth2 -inflate2 -curvstats -sphere -surfreg -jacobian_white -avgcurv -cortparc -hires ${seed_cmd_appendix}

#Highres pial stuff (this module adjusts the pial surface based on the the T2w image)
log_Msg "High Resolution pial surface"
"$PipelineScripts"/FreeSurferHiresPial_freesurfer6_new.sh "$SubjectID" "$SubjectDIR" "$T1wImage" "$T2wImage"

#Final Recon-all Steps
log_Msg "Final Recon-all Steps"
recon-all -subjid $SubjectID -sd $SubjectDIR -surfvolume -parcstats -cortparc2 -parcstats2 -cortparc3 -parcstats3 -cortribbon -segstats -aparc2aseg -wmparc -balabels -label-exvivo-ec -hires ${seed_cmd_appendix}
