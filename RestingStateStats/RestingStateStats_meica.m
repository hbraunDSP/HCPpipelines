...
if(exist(sprintf('%s_hp%d%s',inputdtseries,hp,fileext),'file'))
    BO=ciftiopen(sprintf('%s_hp%d%s',inputdtseries,hp,fileext),WBC);
    BO.cdata=demean(BO.cdata,tpDim);
else
...
fprintf('%s - Starting fslmaths filtering of mixing matrix\n',func_name);
save_avw(reshape(ICAorig',size(ICAorig,2),1,1,size(ICAorig,1)),[outprefix '_fakeNIFTI'],'f',[1 1 1 TR]);
system(sprintf(['fslmaths ' outprefix '_fakeNIFTI -bptf %f -1 ' outprefix '_fakeNIFTI'],0.5*hp/TR));
ICAorig=normalise(reshape(read_avw([outprefix '_fakeNIFTI']),size(ICAorig,2),size(ICAorig,1))');
unix(['rm ' outprefix '_fakeNIFTI.nii.gz']);
fprintf('%s - Finished fslmaths filtering of mixing matrix\n',func_name);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%ICAorig=normalise(load(sprintf(ICAs)));
...
if SaveGrayOrdinateMaps
  ...
  %save maps and convert to dscalar
  %dscalar map names come from wb_command -file-information -only-map-names
  statnamefile=sprintf('%s/cifti_RSS_stats_map_names.txt',justdir(mfilename('fullpath')));
  system(sprintf('rm -f %s %s',[ outprefix '_stats.dtseries.nii'],[outprefix '_stats.dscalar.nii']));
  ciftisave(statscifti,[outprefix '_stats.dtseries.nii'],WBC);
  system(sprintf('%s -cifti-convert-to-scalar %s ROW %s -name-file %s',WBC,...
      [outprefix '_stats.dtseries.nii'],[outprefix '_stats.dscalar.nii'],...
      statnamefile));
  if(exist([outprefix '_stats.dscalar.nii'],'file'))
      system(sprintf('rm -f %s',[ outprefix '_stats.dtseries.nii']));
  else
      error('Conversion of stats.dtseries to stats.dscalar failed!');
  end
end
...
fprintf(fid,'%s\n',varNames);
fprintf(fid,'%s,%d,%d,%d',outprefix,length(Isignal),length(Inoise),length(Isignal)+length(Inoise));
