...
for im2 in $newimlist ; do
	$FSLDIR/bin/fslmaths ${im2}_roi -edge ${im2}_roi_edge
    if [ $im2 != $im1 ] ; then
	...
	$FSLDIR/bin/flirt -in ${im2}_roi_edge -init ${im2}_to_im1.mat -omat ${im2}_to_im1_linmask.mat -out ${im2}_to_im1_linmask -ref ${im1}_roi_edge -refweight ${im1}_roi_linmask -nosearch
