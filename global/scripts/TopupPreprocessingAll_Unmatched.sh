...
TopupDwellTime=`getopt1 "--SEechospacing" $@`  # "$5"
...
if [[ $UnwarpDir = "x" || $UnwarpDir = "x-" || $UnwarpDir = "-x" ]] ; then
  ...
  ro_time=`echo "scale=6; ${TopupDwellTime} * ${nPEsteps}" | bc -l` #Compute Total_readout in secs with up to 6 decimal places
...
if [[ "$scoutdims" == "$topupdims" ]] && [[ $samedwell == 1 ]]; then
	...
	${FSLDIR}/bin/fugue --loadfmap=${WD}/TopupField --dwell=${DwellTime} --saveshift=${WD}/FieldMap_ShiftMap.nii.gz    
