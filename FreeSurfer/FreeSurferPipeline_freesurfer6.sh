...
recon-all -subjid $SubjectID -sd $SubjectDIR -hires -autorecon2 -nosmooth2 -noinflate2 -nocurvstats -nosegstats -openmp ${num_cores}

if [ "$T2wImage" = "NONE" ]; then
    log_Msg "T2w=NONE. Skipping T2w to T1w fine tuning"
    t2regfile="$SubjectDIR/$SubjectID/mri/transforms/T2wtoT1w.mat"

	echo "1 0 0 0" > $t2regfile
	echo "0 1 0 0" >> $t2regfile
	echo "0 0 1 0" >> $t2regfile
	echo "0 0 0 1" >> $t2regfile
fi
...
"$PipelineScripts"/FreeSurferHiresWhite_freesurfer6.sh "$SubjectID" "$SubjectDIR" "$T1wImage" "$T2wImage"

#Intermediate Recon-all Steps
log_Msg "Intermediate Recon-all Steps"
recon-all -subjid $SubjectID -sd $SubjectDIR -hires -smooth2 -inflate2 -curvstats -sphere -surfreg -jacobian_white -avgcurv -cortparc 

#Highres pial stuff (this module adjusts the pial surface based on the the T2w image)
if [ "$T2wImage" = "NONE" ]; then
    log_Msg "T2w=NONE. Skipping High Resolution pial surface"
else
    log_Msg "High Resolution pial surface"
    "$PipelineScripts"/FreeSurferHiresPial_freesurfer6.sh "$SubjectID" "$SubjectDIR" "$T1wImage" "$T2wImage"
fi

#Final Recon-all Steps
log_Msg "Final Recon-all Steps"
recon-all -subjid $SubjectID -sd $SubjectDIR -hires -surfvolume -parcstats -cortparc2 -parcstats2 -cortparc3 -parcstats3 -cortribbon -segstats -aparc2aseg -wmparc -balabels -label-exvivo-ec 
