...
OutputPDwRestoredImage=`getopt1 "--oPDim" $@`  # "$8"
OutputPDwRestoredBrainImage=`getopt1 "--oPDbrain" $@`  # "$9"
...
${FSLDIR}/bin/fslmaths $PDwImage $WD/T1wmulT2w.nii.gz -odt float
...
${FSLDIR}/bin/fslmaths $PDwImage -div $OutputBiasField -mas $T1wImageBrain $OutputPDwRestoredBrainImage -odt float
${FSLDIR}/bin/fslmaths $PDwImage -div $OutputBiasField $OutputPDwRestoredImage -odt float
