...
get_options()
{
	...
	if [ ! -z "${g_bias_field_type}" ]; then
		log_Msg "g_bias_field_type: ${g_bias_field_type}"
	fi
...
main()
{
	...
	######
	g_fmri_name_orig="${g_fmri_name}"
	g_fmri_name="${g_fmri_name}${g_fmri_suffix}"
	#######
	...
	if [ "${g_reg_name}" = "FS" ]; then
		RegString=""
		g_reg_name="reg.reg_LR"
	elif [ "${g_reg_name}" = "MSMSulc" ] || [ "${g_reg_name}" = "NONE" ]; then
		RegString=""
		g_reg_name="MSMSulc"
	else
	...
	if [ "${g_bias_field_type}" = "SEBASED" ]; then
		origBias="${ResultsFolder}/${g_fmri_name_orig}_sebased_bias.nii.gz"
		inputImage="${ResultsFolder}/BiasField.${g_final_fmri_res}.nii.gz"
		cp -f $origBias $inputImage
	else
		...
		referenceImage="${ResultsFolder}/${g_fmri_name_orig}_SBRef.nii.gz"
		...
		if [ "${g_bias_field_type}" = "ONES" ]; then
			#newname=`remove_ext ${inputImage}`.ONES.nii.gz
			${FSLDIR}/bin/fslmaths ${inputImage} -mul 0 -add 1 ${inputImage}
			#newBiasField=${newname}
			#inputImage=${newname}
		fi
	fi
	...
	${FSLDIR}/bin/fslmaths \
 	...
 	for Hemisphere in L R ; do
		...
		# ribbon constraint ROI volume - From Functional Preprocessing
 		roiVolume="${ResultsFolder}/RibbonVolumeToSurfaceMapping${g_fmri_suffix}/goodvoxels.nii.gz"
	...
	echo "$g_brain_ordinates_res == $g_final_fmri_res"
	#if [ 1 -eq `echo "sqrt(($g_brain_ordinates_res - $g_final_fmri_res)^2) < .001" | bc -l` ]; then
	...
	if [ ! ${g_wm} = "NONE" ] ; then
		...
		${CARET7DIR}/wb_command \
			-volume-label-import ${ROIFolder}/wmparc.${g_brain_ordinates_res}.nii.gz ${g_wm} \
		...
		${FSLDIR}/bin/fslmaths ${ROIFolder}/ROIs.${g_brain_ordinates_res}.nii.gz -add ${ROIFolder}/L.${g_final_fmri_res}.nii.gz -add ${ROIFolder}/R.${g_final_fmri_res}.nii.gz -dilD -dilD ${ROIFolder}/WMRegAvoid.${g_final_fmri_res}.nii.gz
	...
	if [ ! ${g_csf} = "NONE" ] ; then
		...
		${CARET7DIR}/wb_command \
			-volume-label-import ${ROIFolder}/wmparc.${g_brain_ordinates_res}.nii.gz ${g_csf} \
		...
		${FSLDIR}/bin/fslmaths ${ROIFolder}/ROIs.${g_brain_ordinates_res}.nii.gz -add ${ROIFolder}/L.${g_final_fmri_res}.nii.gz -add ${ROIFolder}/R.${g_final_fmri_res}.nii.gz -dilD -dilD ${ROIFolder}/CSFRegAvoid.${g_final_fmri_res}.nii.gz
	...
	if [ "X${g_fmri_suffix}" != X ] && [ -e "${ResultsFolder}/Movement_Regressors${g_fmri_suffix}.txt" ]; then
		motionparameters="${ResultsFolder}/Movement_Regressors${g_fmri_suffix}" #No .txt
	fi

	do_skipHPF=false
	#if [ `imtest ${ResultsFolder}/${g_fmri_name}` = 0 ] && [ `imtest ${ResultsFolder}/${g_fmri_name}_hp${g_high_pass}` != 0  ]; then
	#	imln ${ResultsFolder}/${g_fmri_name}_hp${g_high_pass} ${ResultsFolder}/${g_fmri_name}
	#fi
	#if [ `imtest ${ResultsFolder}/${g_fmri_name}_Atlas${RegString}.dtseries` = 0 ] && [ `imtest ${ResultsFolder}/${g_fmri_name}_Atlas${RegString}_hp${g_high_pass}.dtseries` != 0  ]; then
	#	imln ${ResultsFolder}/${g_fmri_name}_Atlas${RegString}_hp${g_high_pass}.dtseries ${ResultsFolder}/${g_fmri_name}_Atlas${RegString}.dtseries
	#	do_skipHPF=true
	#fi
	#
	#if [ 1 -eq `imtest ${ResultsFolder}/${g_fmri_name}_Atlas${RegString}_hp${g_high_pass}.dtseries` ]; then
	#	do_skipHPF=true
	#fi
	...
	RssFolder="${ResultsFolder}/RestingStateStats${g_fmri_suffix}"
	...
	case ${g_matlab_run_mode} in
		...
		1)
			...
			echo "addpath ${HCPPIPEDIR}/global/matlab" >> ${matlab_script_file_name}
			echo "addpath ${FSLDIR}/etc/matlab" >> ${matlab_script_file_name}
			echo "RestingStateStats('${motionparameters}',${g_high_pass},${TR},'${ICAs}','${noise}','${CARET7DIR}/wb_command','${dtseries}','${bias}','${RssPrefix}','${g_dlabel_file}','${g_bc_mode}','${g_out_string}','${WM}','${CSF}',${do_skipHPF});" >> ${matlab_script_file_name}

			log_Msg "About to execute the following Matlab script"

			cat ${matlab_script_file_name}
			#cat ${matlab_script_file_name} | ${HCP_MATLAB_PATH}/bin/matlab -nojvm -nodisplay -nosplash
			cat ${matlab_script_file_name} | ${HCP_MATLAB_PATH}/bin/matlab -nodisplay -nosplash
			;;
